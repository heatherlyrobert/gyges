...
EXIM_export             (char a_style)
{
   ...
   if (strchr ("VDFCTN", a_style) != NULL) {
      fprintf (f, "### generated by gyges-hekatonkheires (hundred-handed) spreadsheet\n");
      fprintf (f, "??? ");
      switch (a_style) {
      case 'V' :  fprintf (f, "V = pure printable values");      break;
   ...
   while (rc >= 0) {
      ...
      ++c;
      /*---(look for line break)---------*/
      if (strchr ("vdfct·VDFCT", a_style) != NULL && x_row != x_rowsave) {
      ...
      if (x_curr == NULL) {
         ...
         switch (a_style) {
         case 'v' : case 'V' :  fprintf (f, "%*.*s", w, w, g_empty);       break;
         case 'd' : case 'D' :  fprintf (f, "%*.*s", w, w, g_empty);     break;
      ...
      /*---(write filled cells)----------*/
         ...
         if (x_curr->v_str != NULL) {
            ...
            strlcpy      (x_modded, x_curr->v_str, LEN_RECD);
         ...
         case 't' : case 'T' :
            ...
            break;
         case 'n' : case 'N' :
            fprintf (f, "cell  %-8.8s  "  , x_curr->label);
            fprintf (f, "%c%c%c%c-  "       , x_curr->align, x_curr->format, x_curr->decs, x_curr->unit);
            fprintf (f, "%s \n"             , x_source);
            ++t;
            break;
         }
   ...
   /*---(close file)---------------------*/
   DEBUG_REGS   yLOG_note    ("closing file");
   if (strchr ("Nn", a_style) == NULL) {
      fprintf (f, "\n");
      ++t;
   }
   if (strchr ("VDFCTN", a_style) != NULL) {
      fprintf (f, "### complete, finito, done.  %d cells, %d occupied, %d lines\n", c, o, t);
   }
   fclose  (f);
   /*---(complete)-----------------------*/
   DEBUG_REGS   yLOG_exit    (__FUNCTION__);
