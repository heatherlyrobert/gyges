...
static const tACTS s_hist_acts [] = {
   ...
   { YMAP_SOURCE   , "change"       , "change contents only"            },
   { YMAP_DELETE   , "delete"       , ""                                },
   { YMAP_CLEAR    , "clear"        , ""                                },
   { YMAP_ALIGN    , "align"        , "display alignment"               },
   { YMAP_FORMAT   , "format"       , "display format"                  },
   { YMAP_DECIMALS , "decimals"     , "display decimal places"          },
   { YMAP_UNITS    , "units"        , "display units of measure"        },
   { YMAP_WIDTH    , "width"        , "change column width"             },
   { YMAP_HEIGHT   , "height"       , "change row height"               },
   { 0             , "end"          , "" },
};
int         s_hist_nact =    0;
 ...
 *>    if (g_before == g_nothing)   HIST__concat (DEF_FORMAT  , a_after , &g_after );                                                 <* 
 *>    else                         HIST__concat (a_beforeF, a_after , &g_after );                                                 <* 
 *>    /+---(add record)---------------------+/                                                                                    <* 
 *>    rc = HIST__new (a_mode, YMAP_SOURCE, a_tab, a_col, a_row);                                                                  <* 
 *>    /+---(complete)--------------------+/                                                                                       <* 
 *>    DEBUG_HIST  yLOG_exit    (__FUNCTION__);                                                                                    <* 
 *>    return rc;                                                                                                                  <* 
 *> }                                                                                                                              <*/
 ...
 *>    rc = HIST__new (a_mode, YMAP_DELETE, a_tab, a_col, a_row);                                                                  <* 
 *>    /+---(complete)--------------------+/                                                                                       <* 
 *>    DEBUG_HIST  yLOG_exit    (__FUNCTION__);                                                                                    <* 
 *>    return rc;                                                                                                                  <* 
 *> }                                                                                                                              <*/
 ...
 *>    HIST__concat (a_beforeF, a_before, &g_before);                                                                              <* 
 *>    HIST__concat (NULL     , NULL    , &g_after );                                                                              <* 
 *>    rc = HIST__new (a_mode, YMAP_CLEAR, a_tab, a_col, a_row);                                                                   <* 
 *>    /+---(complete)--------------------+/                                                                                       <* 
 *>    DEBUG_HIST  yLOG_exit    (__FUNCTION__);                                                                                    <* 
 *>    return rc;                                                                                                                  <* 
 *> }                                                                                                                              <*/
...
HIST__unit         (char *a_question, int a_ref)
{
   ...
   if        (strcmp (a_question, "count"     )    == 0) {
      ...
      snprintf (unit_answer, LEN_FULL, "HIST entry  (%2d) : %s", a_ref, yMAP_mundo_entry (a_ref));
   }
   /*---(complete)-----------------------*/
   return unit_answer;
}
